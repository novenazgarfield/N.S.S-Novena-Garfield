...
class DesktopInterface:
    ...
    def render_system_stats(self):
        ...
        UIComponents.render_system_stats(self.config)
        
        # 使用率进度条
        usage_percent = (st.session_state.query_count / self.config.get("max_daily_queries", 500)) * 100
        st.progress(usage_percent / 100)
        st.caption(f"今日查询使用率: {usage_percent:.1f}%")
